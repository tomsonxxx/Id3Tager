...
const App: React.FC = () => {
    ...
    const handleSaveChanges = async () => {
        ...
        setIsSaving(true);
        filesToSave.forEach(f => updateFileState(f.id, { state: ProcessingState.SAVING }));
        
        let successCount = 0;
        for (const file of filesToSave) {
            try {
                const writable = await file.handle!.createWritable();
                const blobWithTags = await applyID3Tags(file.file, file.fetchedTags || {});
                await writable.write(blobWithTags);
                await writable.close();
                updateFileState(file.id, { state: ProcessingState.SUCCESS });
                successCount++;
            } catch (error) {
                console.error(`Błąd zapisu pliku ${file.file.name}:`, error);
                updateFileState(file.id, { state: ProcessingState.ERROR, errorMessage: `Błąd zapisu: ${error instanceof Error ? error.message : 'Nieznany błąd'}` });
            }
        }
        setIsSaving(false);
    ...
    const handleDownloadSelected = async () => {
        ...
        if (filesToDownload.length === 1) {
            ...
            const blob = await applyID3Tags(file.file, file.fetchedTags!);
