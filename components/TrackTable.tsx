...
const columns: ColumnDef[] = [
    ...
    { id: 'title', label: 'Tytuł', defaultWidth: 220, minWidth: 150, isSortable: true },
    { id: 'artist', label: 'Artysta', defaultWidth: 160, minWidth: 100, isSortable: true },
    { id: 'bpm', label: 'BPM', defaultWidth: 60, minWidth: 50, isSortable: true },
    { id: 'key', label: 'Tonacja', defaultWidth: 70, minWidth: 60, isSortable: true },
    ...
    { id: 'dateAdded', label: 'Dodano', defaultWidth: 120, minWidth: 100, isSortable: true },
];
...
const TrackTable: React.FC<TrackTableProps> = ({ 
  ...
  // Note: 'allSelected' logic here assumes we only select visible items for the checkbox visually
  // but logic in App/useLibrary usually selects *all* available in the filter.
  // For UI consistency, we check if all visible files are selected.
  ...
  const renderStars = (rating: number | undefined) => {
      ...
      for (let i = 1; i <= 5; i++) {
          stars.push(
              <span key={i} className={i <= val ? "text-yellow-400" : "text-slate-300 dark:text-slate-700"}>★</span>
          );
      }
  ...
  return (
    <div className="flex-grow flex flex-col overflow-hidden bg-white dark:bg-slate-900 select-none">
            ...
            <div className="bg-white dark:bg-slate-900 divide-y divide-slate-200 dark:divide-slate-800">
                {files.map((file) => {
                    ...
                    >
                        ...
                        <div className="px-4 py-2 flex-shrink-0 text-xs text-slate-400 dark:text-slate-500" style={{ width: columnWidths['dateAdded'] }}>
                            {date}
                        </div>
                    </div>
                    );
      ...
      {totalPages > 1 && onPageChange && (
          <div className="flex items-center justify-between px-4 py-2 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 text-xs">
