...
import { GoogleGenAI, Type, GenerateContentResponse, SchemaType } from "@google/genai";
...
-   **Format:** Return pure JSON matching the schema.
...
const singleFileSchemaProperties = {
    originalFilename: { type: Type.STRING, description: "EXACT filename from input list (for mapping)." },
    ...
    encodedBy: { type: Type.STRING },
    originalArtist: { type: Type.STRING },
    discNumber: { type: Type.STRING },
    albumCoverUrl: { type: Type.STRING, description: "Direct URL to high-res cover art found via search." },
    mood: { type: Type.STRING },
    comments: { type: Type.STRING },
    isrc: { type: Type.STRING },
    recordLabel: { type: Type.STRING },
    bpm: { type: Type.NUMBER, description: "Beats Per Minute (Integer)" },
    initialKey: { type: Type.STRING, description: "Musical Key in Camelot Notation (e.g. 11B)" },
    energy: { type: Type.NUMBER, description: "Energy Level 1-10" },
    danceability: { type: Type.NUMBER, description: "Danceability Level 1-10" },
    releaseType: { type: Type.STRING, enum: ['album', 'single', 'compilation', 'ep', 'remix'] },
...
const batchResponseSchema = {
    ...
    items: {
        type: Type.OBJECT,
        properties: singleFileSchemaProperties,
        required: ["originalFilename", "artist", "title"],
    }
};
